	...
	public void run() {
		...
		while (running) {
			...
			try {
				...
				if (fix != null && (fix.length() - 2 == scanresults.size()))// wifi
																			// signal
																			// changed
				{
					for (ScanResult sr : scanresults) {
						...
						&& (fix.length() - 2 != scanresults.size())) // number
																		// of
																		// available
																		// wifi
																		// changed
					flag = true;
				// only when # of wifi changed or wifi signal changes, reset fix
				// and notify observer, otherwise, sleep and wait to check next
				// return
				// Log.v("flag",
				// flag+" (true mean need update, false means no need to update)");
				if (flag) {
					fix = new JSONObject();
					fix.put("sensor", 2.0);
					fix.put("ts", timestamp);
